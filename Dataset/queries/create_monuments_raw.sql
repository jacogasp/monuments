create table if not exists monuments_raw as (
    with osm_points as (
        select osm_id,
               name,
               amenity,
               case
                   when historic = 'cemetery' then 'tomb'
                   when historic = 'wayside_shrine' then 'tomb'
                   else historic
                   end as historic,
               tourism,
               "addr:housename",
        ...
        where name is not null
          and (historic in ('memorial', 'wayside_shrine', 'monument', 'archaeological_site', 'ruins',
                            ...
                            'monastery', 'aqueduct', 'bunker', 'citywalls', 'bridge', 'chapel', 'tower')
            or amenity in
    ...
    select *
    from osm_polygons
    where amenity in (select category from categories) or
    historic in (select category from categories) or
    tourism in (select category from categories)
);
